...
/*
void* invert(gsl_matrix* A,gsl_matrix* Ai){
        int Nf = A->size2;
        gsl_matrix* R=gsl_matrix_alloc(Nf,Nf);
        GS_decomp(A,R);
        int n=A->size1;
	...
	gsl_blas_dgemv(CblasTrans,1.0,A,ei,0.0,x);
	backsub(R,aii);
...
void* fit (gsl_matrix* data, double func(int i, double x), int Nf, gsl_vector* y, gsl_matrix* Covy ){
        int N=data->size1;
	gsl_matrix* A=gsl_matrix_alloc(N,Nf);
	gsl_matrix* R=gsl_matrix_alloc(Nf,Nf);
	gsl_matrix* Rr=gsl_matrix_alloc(Nf,Nf);
	gsl_matrix* Ri=gsl_matrix_alloc(Nf,Nf);
	gsl_vector* Y=gsl_vector_alloc(N);
//	gsl_matrix* Rr=gsl_matrix_alloc(N,Nf);	
